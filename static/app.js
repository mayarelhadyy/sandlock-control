...
function fmtTime(s){if(!s||s==='Not recorded')return s||'—';const d=new Date(s);if(Number.isNaN(d.getTime()))return s;return new Intl.DateTimeFormat('en-GB',{timeZone:'Africa/Cairo',day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:true}).format(d)}
function displayCell(key,value){return ['Created At','Updated At','Completed At','Cancelled At','Timestamp','Submitted At','Account Created'].includes(key)?fmtTime(value):value}
...
async function loadOverview(){const generation=++overviewGeneration;const d=await json('/api/overview');if(generation!==overviewGeneration||current!=='overview')return;const st=d.state||{},l=st.locker||{};$('#kUsers').textContent=d.kpi.users;$('#kBookings').textContent=d.kpi.activeReservations;$('#kAlerts').textContent=d.kpi.alerts;$('#kRevenue').textContent=`${Number(d.kpi.demoBookingValue||0).toFixed(2)} EGP`;$('#kRevenue').title=`Demo only, including final late-fee assessments. Excludes ${d.kpi.operatorReviewBookings||0} operator-review and ${d.kpi.legacyReviewBookings||0} legacy-review bookings. Actual Collected Revenue: N/A`;const brokerState=String(st.brokerStatus||(st.brokerConnected?'connected':'disconnected')).toLowerCase();$('#brokerDot').classList.toggle('on',brokerState==='connected');$('#brokerText').textContent=({connected:'Connected',connecting:'Connecting…',reconnecting:'Reconnecting…',disconnected:'Disconnected'})[brokerState]||'Disconnected';$('#liveStatus').textContent=pretty(l.status);$('#liveDoor').textContent=pretty(l.door);$('#liveBattery').textContent=l.battery==null?'—':`${l.battery}%`;$('#liveLast').textContent=fmtTime(l.lastSeen);const b=$('#liveBadge');b.className=`badge ${statusBadge(l.online?l.status:'offline')}`;b.textContent=l.online?String(l.status||'online').toUpperCase():'OFFLINE';const p=$('#pinA');p.className=`pin pA ${statusBadge(l.online?l.status:'offline')}`;renderMini('#activeRes',d.activeReservations,r=>[`Locker ${r.Locker||'A'} • ${r['User Name']||r['User Mobile']||'User'}`,`${r.Date||''} • ${r['Start Time']||''}–${r['End Time']||''} • ${r.Status||''}`]);renderMini('#latestAlerts',d.alerts,a=>[a['Alert Type']||'Alert',`${a.Severity||''} • ${a.Message||''}`]);}
...
const titles={reservations:['RESERVATIONS','Reservations'],users:['USERS','Users'],payments:['PAYMENTS','Payments'],feedback:['FEEDBACK','Feedback'],alerts:['ALERTS','Alerts']};
...
function columnLabel(key){return key;}
...
function renderTable(data){
 ...
 if(!data?.length){tbody.innerHTML=`<tr><td>${current==='feedback'?'No feedback yet.':'No matching records.'}</td></tr>`;return;}
 ...
 for(const r of data){
  const row=document.createElement('tr');row.innerHTML=keys.map(k=>`<td>${esc(displayCell(k,r[k]))}</td>`).join('');
